<?php
/*
* ZPPS Post Box Widget
*/

class ZPPS_Post_Carousel extends WP_Widget {
	/**
	...
	function __construct() {
		...
		$this->defaults = array(
			...
			'content_limit'           => '',
			'more_text'               => __( 'Read More...', 'zp-post-slider' ),
		);

		$widget_ops = array(
			'classname'   => 'zpps-post-carousel',
			'description' => __( 'Displays featured posts with carousel', 'zp-post-slider' ),
		);

		$control_ops = array(
			'id_base' => 'zpps-post-carousel',
			'width'   => 505,
			'height'  => 350,
		);

		parent::__construct( 'zpps-post-carousel', __( 'ZP - Post Carousel', 'zp-post-slider' ), $widget_ops, $control_ops );
	...
	function widget( $args, $instance ) {
		...
		$data_attr_line = 'class="zpps-widget-swiper swiper-container"';
				...
				echo '<div class="swiper-wrapper">';
					...
					if ( have_posts() ) : while ( have_posts() ) : the_post();
						...
						$image = genesis_get_image( array(
							...
							'attr'    => array ( 'alt' => get_the_title(), 'class' => 'alignleft' ),
						) );
						...
						if ( $instance['show_title'] )
							...
							if ( ! empty( $instance['show_title'] ) ) {
								...
								$title = get_the_title() ? get_the_title() : __( '(no title)', 'zp-post-slider' );
							...
							if ( $instance['show_date']  || $instance['show_author']  || $instance['show_comments']  ){
								echo '<p class="entry-meta">';
									if( $instance['show_date'] )
										echo '<span class="entry_date_wrap"><span class="dashicons dashicons-calendar-alt"></span>'.do_shortcode( '[post_date]' ).'</span>';

									if( $instance['show_author'] )
										echo '<span class="entry_author_wrap">'.$gravatar.do_shortcode( '[post_author_posts_link]' ).'</span>';

									if( $instance['show_comments'] )
										echo '<span class="entry_comment_wrap"><span class="dashicons dashicons-admin-comments"></span>'.do_shortcode( '[post_comments]' ).'</span>';
								echo '</p>';
							}
						...
						if ( $instance['show_readmore'] ) {
							$morelink = ( !empty( $instance['more_text']  ) )? $instance['more_text'] : __( 'Read More', 'zp-post-slider' );
	...
	function form( $instance ) {
		...
		?>
		<p>
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title', 'zp-post-slider' ); ?>:</label>
		...
		<div class="genesis-widget-column">
			...
			<div class="genesis-widget-column-box genesis-widget-column-box-top">
				...
				<p>
					<label for="<?php echo $this->get_field_id( 'posts_cat' ); ?>"><?php _e( 'Category', 'zp-post-slider' ); ?>:</label>
					<?php
					$categories_args = array(
						...
						'orderby'         => 'Name',
						'hierarchical'    => 1,
						'show_option_all' => __( 'All Categories', 'zp-post-slider' ),
						'hide_empty'      => '0',
					);
				...
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'posts_num' ) ); ?>"><?php _e( 'Number of Posts to Show', 'zp-post-slider' ); ?>:</label>
				...
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'posts_offset' ) ); ?>"><?php _e( 'Number of Posts to Offset', 'zp-post-slider' ); ?>:</label>
				...
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'orderby' ) ); ?>"><?php _e( 'Order By', 'zp-post-slider' ); ?>:</label>
					<select id="<?php echo esc_attr( $this->get_field_id( 'orderby' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'orderby' ) ); ?>">
						<option value="date" <?php selected( 'date', $instance['orderby'] ); ?>><?php _e( 'Date', 'zp-post-slider' ); ?></option>
						<option value="title" <?php selected( 'title', $instance['orderby'] ); ?>><?php _e( 'Title', 'zp-post-slider' ); ?></option>
						<option value="parent" <?php selected( 'parent', $instance['orderby'] ); ?>><?php _e( 'Parent', 'zp-post-slider' ); ?></option>
						<option value="ID" <?php selected( 'ID', $instance['orderby'] ); ?>><?php _e( 'ID', 'zp-post-slider' ); ?></option>
						<option value="comment_count" <?php selected( 'comment_count', $instance['orderby'] ); ?>><?php _e( 'Comment Count', 'zp-post-slider' ); ?></option>
						<option value="rand" <?php selected( 'rand', $instance['orderby'] ); ?>><?php _e( 'Random', 'zp-post-slider' ); ?></option>
					</select>
				</p>

				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'order' ) ); ?>"><?php _e( 'Sort Order', 'zp-post-slider' ); ?>:</label>
					<select id="<?php echo esc_attr( $this->get_field_id( 'order' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'order' ) ); ?>">
						<option value="DESC" <?php selected( 'DESC', $instance['order'] ); ?>><?php _e( 'Descending (3, 2, 1)', 'zp-post-slider' ); ?></option>
						<option value="ASC" <?php selected( 'ASC', $instance['order'] ); ?>><?php _e( 'Ascending (1, 2, 3)', 'zp-post-slider' ); ?></option>
					</select>
				</p>

				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'image_size' ) ); ?>"><?php _e( 'Image Size', 'zp-post-slider' ); ?>:</label>
			...
			<div class="genesis-widget-column-box">
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'slides_per_view' ) ); ?>"><?php _e( 'Slides per view', 'zp-post-slider' ); ?>:</label>
				...
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'space_between_slides' ) ); ?>"><?php _e( 'Width of the space between slides(px)', 'zp-post-slider' ); ?>:</label>
				...
				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'duration_speed' ) ); ?>"><?php _e( 'Duration of transition between slides (ms)', 'zp-post-slider' ); ?>:</label>
				...
				<p>
					...
					<label for="<?php echo esc_attr( $this->get_field_id( 'navigation' ) ); ?>"><?php _e( 'Slider navigation', 'zp-post-slider' ); ?></label>
				</p>
				<p>
					...
					<label for="<?php echo esc_attr( $this->get_field_id( 'pagination' ) ); ?>"><?php _e( 'Slider pagination', 'zp-post-slider' ); ?></label>
				</p>
			</div>
		</div>
		...
		<div class="genesis-widget-column genesis-widget-column-right">
			...
			<div class="genesis-widget-column-box genesis-widget-column-box-top">
				...
				<p>
					...
					<label for="<?php echo esc_attr( $this->get_field_id( 'show_title' ) ); ?>"><?php _e( 'Show Post Title', 'zp-post-slider' ); ?></label>
				</p>
				<p>
					...
					<label for="<?php echo esc_attr( $this->get_field_id( 'show_date' ) ); ?>"><?php _e( 'Show Post Date', 'zp-post-slider' ); ?></label>
				</p>
				...
				<p>
					...
					<label for="<?php echo esc_attr( $this->get_field_id( 'show_author' ) ); ?>"><?php _e( 'Show Post Author', 'zp-post-slider' ); ?></label>
				</p>
				...
				<p>
					...
					<label for="<?php echo esc_attr( $this->get_field_id( 'show_comments' ) ); ?>"><?php _e( 'Show Post Comments', 'zp-post-slider' ); ?></label>
				</p>

				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'show_content' ) ); ?>"><?php _e( 'Content Type', 'zp-post-slider' ); ?>:</label>
					<select id="<?php echo esc_attr( $this->get_field_id( 'show_content' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'show_content' ) ); ?>">
						<option value="content" <?php selected( 'content', $instance['show_content'] ); ?>><?php _e( 'Show Content', 'zp-post-slider' ); ?></option>
						<option value="excerpt" <?php selected( 'excerpt', $instance['show_content'] ); ?>><?php _e( 'Show Excerpt', 'zp-post-slider' ); ?></option>
						<option value="content-limit" <?php selected( 'content-limit', $instance['show_content'] ); ?>><?php _e( 'Show Content Limit', 'zp-post-slider' ); ?></option>
						<option value="" <?php selected( '', $instance['show_content'] ); ?>><?php _e( 'No Content', 'zp-post-slider' ); ?></option>
					</select>
					<br />
					<label for="<?php echo esc_attr( $this->get_field_id( 'content_limit' ) ); ?>"><?php _e( 'Limit content to', 'zp-post-slider' ); ?>
						...
						<?php _e( 'characters', 'zp-post-slider' ); ?>
					</label>
				</p>
				...
				<p>
					...
					<label for="<?php echo esc_attr( $this->get_field_id( 'show_readmore' ) ); ?>"><?php _e( 'Show Read More?', 'zp-post-slider' ); ?></label>
				</p>

				<p>
					<label for="<?php echo esc_attr( $this->get_field_id( 'more_text' ) ); ?>"><?php _e( 'More Text', 'zp-post-slider' ); ?>:</label>
	...
	function enqueue_style_js(){
		...
		wp_enqueue_style( 'zpps-style' );
		wp_enqueue_script( 'jquery-swiper' );
		wp_enqueue_script( 'zpps-init' );	
	}
